...
# collect LLM examples
examples = []
for p in sorted(EX_LLM.glob("example*_*.instagram.txt")):
    base = p.with_suffix("")  # remove .txt
...
COL_BG = RGBColor(10, 24, 48)      # navy
COL_ACC = RGBColor(0, 163, 163)    # teal
COL_LIGHT = RGBColor(250,250,252)  # light
...
def footer(slide, n):
    ...
    for run in p.runs: run.font.size=Pt(10); run.font.color.rgb=COL_DKTXT
...
p = tb.paragraphs[0]; p.text = "Biotact Q4-2025 Marketing Plan + IE-Agent (LLM)"
for r in p.runs: r.font.size=Pt(44); r.font.bold=True; r.font.color.rgb=COL_TEXT
p2 = tb.add_paragraph(); p2.text = "12-week plan • LLM examples • AR prompts"
...
# Example slides (IG/Podcast/Email + AR json)
for idx, ex in enumerate(examples, start=4):
    ...
    p2 = left.add_paragraph(); p2.text = (ex["ig"][:1000] + " …") if len(ex["ig"])>1000 else ex["ig"]
